...
export function updateIdxOfArray<T>(idx: number, arr: Array<T>, newValue: T): Array<T> {
  arr[idx] = newValue;
...
export function compareDates(dateA: Date, dateB: Date): boolean {
  const [monthA, dayA, yearA] = [dateA.getMonth(), dateA.getDate(), dateA.getFullYear()];
  const [monthB, dayB, yearB] = [dateB.getMonth(), dateB.getDate(), dateB.getFullYear()];
